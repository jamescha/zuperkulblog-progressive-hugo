...
page('/:category/list', function(ctx) {
  ...
  // set target to our Hugo path
  var category = ctx.params.category + '/index.json';
...
page('/:category/detail/:slug', function(ctx) {
  ...
  // set our target to our Hugo path
  var target = ctx.params.category + '/detail/' + ctx.params.slug + '/index.json';
  
  ajax(target, function(err, data) {
    function setData() {
      blog.page = 'detail';
      blog.category = category;
      blog.article = data;
